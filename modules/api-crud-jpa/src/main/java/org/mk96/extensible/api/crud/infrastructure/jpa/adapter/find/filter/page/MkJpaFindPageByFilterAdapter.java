package org.mk96.extensible.api.crud.infrastructure.jpa.adapter.find.filter.page;
...
public class MkJpaFindPageByFilterAdapter<DOM extends MkDomain, EN extends MkEntity, ID>
    extends AbstractMkAdapter<DOM>
	implements MkFindPageByFilterPort<DOM> {
	...
	@Override
	@Transactional
	public List<DOM> findPageByFilter(MkFilter<DOM> domFilter, Pageable pageable) {

        MkFilter<EN> enFilter = domFilter.map(mapper::map);
        Specification<EN> enSpec = MkJpaFilterUtils.buildSpec(enFilter);
        List<EN> entities = repository.findAll(enSpec, pageable).getContent();
