package org.mk96.extensible.api.crud.infrastructure.jpa.adapter.save.merge.all;
...
public class MkJpaMergeAllAdapter<DOM extends MkDomain, EN extends MkEntity, ID>
    extends AbstractMkAdapter<DOM>
    implements MkMergeAllPort<DOM> {
    ...
    private Stream<DOM> mergeMany(MkMergeSave<DOM> domMerge) {
        ...
        return ensMerged.stream().map(mapper::map);
    }

    @Override
    @Transactional
    public List<DOM> mergeAll(List<MkMergeSave<DOM>> merges) {
        return merges.stream().flatMap(this::mergeMany).toList();
    }
