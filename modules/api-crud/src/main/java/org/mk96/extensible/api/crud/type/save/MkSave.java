...
public abstract class MkSave<T extends MkObject> extends MkDomain {
	...
	protected final MkSaveType saveType;

    public abstract <Tp extends MkObject> MkSave<Tp> map(Function<T, Tp> mapper);

    public static <T extends MkObject, Tp extends MkObject> Function<MkSave<T>, MkSave<Tp>> transform(Function<T, Tp> mapper) {
        return save -> save.map(mapper);
    }
